...
// =============================================================================
// REGIME DETECTION (mirrors live HMM + PI cycle + Markov logic)
// =============================================================================
const regimeCache = new Map();

function detectRegimeBacktest(btcDaily, upToIndex) {
  const key = Math.max(0, upToIndex);
  if (regimeCache.has(key)) return regimeCache.get(key);

  const slice  = btcDaily.slice(0, key + 1);
  ...
  const piCycle = (() => {
    ...
    const ratio = m111 / m350x2;
    if (ratio >= 0.98) return "top";
    if (ratio >= 0.90) return "late_bull";
    return "bull";
  })();
  ...
  const hmmParams = initHMMParams(returns);
  const { hmmState } = viterbiHMM(returns, hmmParams);
  const hmmLabel = hmmState === 0 ? "bull" : "bear";

  const markovChain = { transitions: [[0.8, 0.2], [0.2, 0.8]] };
  updateMarkovChain(markovChain, returns);
  const markovProb = markovChain.transitions[hmmState === 0 ? 0 : 1][0];
  ...
  let label;
  if (range < 0.10) label = "sideways";
  else if (bull >= 2) label = "bull";
  else label = "bear";

  regimeCache.set(key, label);
  return label;
}
...
function viterbiHMM(observations, params) {
  ...
  const updatedParams = { ...params, means: [...params.means], stds: [...params.stds] };
  for (let s = 0; s < K; s++) {
    const obs = observations.filter((_, i) => path[i] === s);
    if (obs.length > 5) {
      updatedParams.means[s] = mean(obs);
      updatedParams.stds[s] = Math.max(std(obs), 0.001);
    }
  }
  return { hmmState: path[T - 1], updatedParams };
}
...
function updateMarkovChain(mc, returns) {
  ...
  for (let i = 1; i < win.length; i++) {
    const prev = win[i - 1] >= 0 ? 0 : 1;
    const curr = win[i] >= 0 ? 0 : 1;
    counts[prev][curr]++;
  }
  for (let s = 0; s < 2; s++) {
    const total = counts[s][0] + counts[s][1];
    if (total > 0) {
      mc.transitions[s][0] = counts[s][0] / total;
      mc.transitions[s][1] = counts[s][1] / total;
    }
  }
}
...
async function backtestSymbol(symbol, candles1h, candles4h, btcDaily, cap) {
  ...
  while (bar < candles1h.length - 10) {
    ...
    const regime = detectRegimeBacktest(btcDaily, btcEnd > 0 ? btcEnd : 0);
