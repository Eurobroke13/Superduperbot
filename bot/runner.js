...
async function phaseRegimeAndExits(env, state, deps) {
  ...
  const [btcDaily, btc4h] = await Promise.all([
    fetchCandles("BTC-USDT-SWAP", "1d", 500),
    fetchCandles("BTC-USDT-SWAP", "4H", 100)
  ]);
  ...
  const regime = detectRegime(btcDaily, state, btc4h || null);
  state.lastRegime = regime;
  console.log(`[REGIME] ${regime.label} | scores: bull=${regime.scores?.bull} bear=${regime.scores?.bear} sw=${regime.scores?.sideways} | HMM:${regime.hmmLabel} | PI:${regime.piCycle} | Markov:${regime.markovProb.toFixed(3)}`);
