...
**3. Structure-aware chandelier trailing exit — BUILT, VALIDATED, REVERTED.** Added a 15m chandelier+S/R trail (engage at +1 ATR, trail to `peak − 2.5×ATR` or just beyond nearest swing S/R) to lock profit earlier and cut losers on structure breaks. The walk-forward A/B (trail OFF vs ON, 6mo, 5 folds) **rejected it**: on the out-of-sample fold, OFF beat ON on every metric — EV **$8.64 vs $4.36**, PF **1.80 vs 1.47**, WR 50.9% vs 47.9% — and OFF won on all 5 folds. The trail clips the TP2/TP3 fat tail and churns into more low-quality re-entries (1661→1918 trades). Since the backtest trails on 1h and live would trail on 15m (even more active = even more clipping), live would be worse. **Reverted entirely** (`exits.js`/`runner.js`/`backtest.js`/`config.js`, removed `tests/chandelier-trail.test.js`); the existing post-TP2 graduated trail is kept. **This is the harness working as designed** — a theoretically-sound change that empirically destroys EV, caught before shipping. Lesson: don't tune the trail to rescue it (overfitting the window); if revisited, start a fresh hypothesis and judge it OOS.
