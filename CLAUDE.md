...
### Fixed
- **Postgres restarts crashed the bot; no retry, no pool error handler (Sept 2026).** Runner logs 2026-09-01 08:47 showed `[task-runner] the database system is in recovery mode` — the fast-scan died before printing `=== BOT RUN ===`, then sat 3.5 min before `Stopping Container`. Postgres SQLSTATE **57P03** = the server is replaying WAL after a restart and refuses new connections for ~10-60s. Four separate defects turned that blip into a crashed deployment: (1) **no `pool.on("error")` listener in `db.js`** — when PG restarts, every idle socket in the pool errors, and an unhandled `'error'` event on an EventEmitter **kills the Node process outright**; `try/catch` around queries cannot catch it, so the always-on server just vanished; (2) **zero retry on any DB call** — a single transient error lost the entire run; (3) `task-runner.js` exited **1** on a DB blip, so Railway burned `restartPolicyMaxRetries: 3` and flagged the cron deployment **CRASHED** (the alert emails); (4) `closeDb()` awaited an unbounded `pool.end()`, which never settles when a client is wedged on a dead socket — that is the 3.5-min limbo. **Fix (`db.js`, `task-runner.js`, `server.js`):** added `pool.on("error")`; `isTransientDbError()` (57P01/02/03, 53300, 08xxx, ECONNRESET/ETIMEDOUT/EPIPE + message match) and `withDbRetry()` with exponential backoff (~38s total, tunable via `DB_RETRY_ATTEMPTS`/`DB_RETRY_BASE_MS`) wrapping `initDb` and both `pool.connect()` sites; transaction **bodies deliberately not retried** (trade inserts aren't idempotent — `_pendingTrades` stays buffered for the next run); advisory-unlock failures no longer mask the real error from inside `finally`; `closeDb()` bounded to 5s. Also `/health` now calls new `pingDb()` instead of `initDb()` — `initDb` short-circuits on its `initialized` flag, so after the first success it **stopped touching Postgres and reported `database: "connected"` for a DB that had been dead for hours**. Transient DB failure now exits **0** on the cron runner (next tick retries) while real bugs still exit 1. **Still open:** *why* Railway PG restarts so often — check `pg_postmaster_start_time()`, DB size and dead-tuple bloat on `bot_state` (one row rewritten every ~2-5 min by two services is a TOAST/autovacuum bloat generator).
