...
4. **`MEAN_REVERSION_PRIMARY = true`** (the MR pivot) — non-MR setups must clear the adaptive Claude threshold (base 6) to even be considered; below that they're rejected `mr-primary-mode`. MR setups always pass this gate. Net effect: the bot trades mean-reversion freely and only lets genuinely strong, Claude-vetted trend/breakout setups through. Driven by the per-signal lift analysis (below).
